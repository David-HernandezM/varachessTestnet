...
/* 
#[test]
fn vara_chess_bad_init_test(){
    let sys = System::new();
    sys.init_logger();
    let program = Program::current(&sys);
    /***********************************************************************/
    //Send incorrect message to INIT of smart contract (GameEnd) ***
    let result_game_tes: ResultEnd = ResultEnd::Win;
    let game_end_test: GameEnd = GameEnd { game_id: 123, result_game:result_game_tes};

    let res=program.send(2, ChessMessageIn::EndGame(game_end_test));
    //Evaluate result
    let expected_log=Log::builder()
        .dest(2)
        .payload(String::from("Bad INIT Message, can't be of type EndGame"));
    //assert!(res.contains(&expected_log));
}
*/
